...
from ..schemas import (
    AssistantModelOut, ChatAdminSessionOut, ChatAdminStatsOut, ChatMessageOut, ChatSendIn, ChatSendOut,
...
def _message_out(message: ChatMessage) -> ChatMessageOut:
    return ChatMessageOut(
        ...
        cost_usd=message.cost_usd, latency_ms=message.latency_ms, created_at=message.created_at,
    )
...
def chat(payload: ChatSendIn, request: Request, user: User = Depends(current_user),
    ...
    chain = _chain_or_503(db, user, request.app.state.secret_key)
    ...
    try:
        result = run_agent_turn(payload.message, history, ordered, make_executor(db, user),
...
def admin_stats(days: int = Query(30, ge=1, le=365), user: User = Depends(require_admin),
    ...
    return ChatAdminStatsOut(
        ...
        completion_tokens=totals[2], cost_usd=totals[3],
