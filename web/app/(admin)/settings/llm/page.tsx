...
const fromServer = (p: LlmProvider): ProviderForm => ({
  uid: nextUid++,
  name: p.name,
  base_url: p.base_url,
  model: p.model,
...
export default function LlmSettingsPage() {
  ...
  useEffect(() => {
    ...
    setForms(list.length ? list.map(fromServer) : [blankForm("Chính")]);
  }, [settings.data]);
  ...
  const save = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setForms(data.providers.length ? data.providers.map(fromServer) : [blankForm("Chính")]);
      setTests({});
    },
  });
    ...
    <>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <form onSubmit={submit} className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {forms.length > 1 && (
                      <button
                        ...
                        onClick={() => setForms((list) => (list ?? []).filter((x) => x.uid !== f.uid))}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button type="button" variant="secondary" loading={check.isPending}
                      disabled={!saved?.has_key} onClick={() => check.mutate(i)}>
                      <PlugZap className="size-4" /> Kiểm tra nhà cung cấp này
                    </Button>
                    {!saved?.has_key && <span className="text-xs text-muted">Lưu khóa trước rồi mới kiểm tra được.</span>}
                  </div>
                  {tests[i] && <TestResult test={tests[i]} />}
                </div>
              </Card>
            );
          })}
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="submit" loading={save.isPending}>Lưu toàn bộ chuỗi</Button>
          ...
          <p className="text-xs text-muted">“Kiểm tra” chỉ gửi vài token bằng cấu hình <strong>đã lưu</strong> và không tính vào thống kê chi phí.</p>
        </form>
